...
@Service
public class CheckBalanceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CheckBalanceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.checkbalance.hosturl}")
	private String url;
	
	@Value("${spring.api.checkbalance.context-path}")
	private String contextPath;

	@Value("${spring.api.checkbalance.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CheckBalanceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CheckBalanceRestProcessor.class.getName());
