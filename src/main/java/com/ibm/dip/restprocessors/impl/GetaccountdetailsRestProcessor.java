...
@Service
public class GetaccountdetailsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(GetaccountdetailsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.getaccountdetails.hosturl}")
	private String url;
	
	@Value("${spring.api.getaccountdetails.context-path}")
	private String contextPath;

	@Value("${spring.api.getaccountdetails.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  GetaccountdetailsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ GetaccountdetailsRestProcessor.class.getName());
