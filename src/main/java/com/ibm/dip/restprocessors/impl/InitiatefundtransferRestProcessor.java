...
@Service
public class InitiatefundtransferRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InitiatefundtransferRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.initiatefundtransfer.hosturl}")
	private String url;
	
	@Value("${spring.api.initiatefundtransfer.context-path}")
	private String contextPath;

	@Value("${spring.api.initiatefundtransfer.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InitiatefundtransferRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InitiatefundtransferRestProcessor.class.getName());
